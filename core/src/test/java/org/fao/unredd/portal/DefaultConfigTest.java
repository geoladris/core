...
public class DefaultConfigTest {
	...
	@Test
	public void testConfigurationProvidersMergeDeprected() throws Exception {
		...
		when(provider1.getConfigurationMap(
				any(PortalRequestConfiguration.class),
				any(HttpServletRequest.class)))
						.thenReturn(buildMap("myModule", conf1));
		ModuleConfigurationProvider provider2 = mock(
				ModuleConfigurationProvider.class);
		when(provider2.getConfigurationMap(
				any(PortalRequestConfiguration.class),
				any(HttpServletRequest.class)))
						.thenReturn(buildMap("myModule", conf2));
		config.addModuleConfigurationProvider(provider1);
		config.addModuleConfigurationProvider(provider2);

		JSONObject pluginConf = config
				.getPluginConfiguration(Locale.getDefault(),
	...
	private Map<String, JSONObject> buildMap(String pluginName,
			JSONObject conf) {
	...
	@Test
	public void testCacheDeprecated() throws Exception {
		...
		Config config = buildConfigReadOnceAndChangeFolderConfigDeprecated(true,
	...
	@Test
	public void testNoCacheDeprecated() throws Exception {
		String defaultLang = "es";
		Locale locale = new Locale(defaultLang);
		ResourceBundle resourceBundle = mock(ResourceBundle.class);
		Properties firstProperties = new Properties();
		firstProperties.put("languages", "{\"es\": \"Espa\u00f1ol\"}");
		firstProperties.put("languages.default", defaultLang);
		Config config = buildConfigReadOnceAndChangeFolderConfigDeprecated(
				false, defaultLang, locale, resourceBundle, firstProperties);
	...
	@Deprecated
	private Config buildConfigReadOnceAndChangeFolderConfigDeprecated(
			boolean useCache, String defaultLang, Locale locale,
			ResourceBundle resourceBundle, Properties firstProperties) {
	...
	public void testPluginConfigurationCached() throws Exception {
		...
		readPluginConfigurationTwiceDeprecated(true, true, 1);
	}
	...
	public void testPluginConfigurationCacheIgnoredIfProviderCannotBeCached()
		...
		readPluginConfigurationTwiceDeprecated(true, false, 2);
	}
	...
	public void testPluginConfigurationCacheIgnoredIfCacheDisabled()
		...
		readPluginConfigurationTwiceDeprecated(false, true, 2);
	}

	private void readPluginConfigurationTwiceDeprecated(boolean useCache,
	...
	@Test
	public void testNoConfigurationFolderDeprecated() {
		Config config = new DefaultConfig(
				new ConfigFolder("doesnotexist", "doesnotexist"), false);
	...
	@Test
	public void testFailingConfigurationProviderDeprecated() throws Exception {
		Config config = new DefaultConfig(
				new ConfigFolder("doesnotexist", "doesnotexist"), false);
		ModuleConfigurationProvider provider = mock(
				ModuleConfigurationProvider.class);
		when(provider.getConfigurationMap(any(PortalRequestConfiguration.class),
				any(HttpServletRequest.class)))
