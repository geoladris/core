...
public class JEEContextAnalyzer {
  ...
  private void scanClasses(Context context) {
    ...
    ClassesPluginLayout pluginLayout = new ClassesPluginLayout(context.getClientRoot());
    if (pluginLayout.rootFolderExists()) {
      this.currentPlugin = new PluginDescriptor();
      extractInfo(context, pluginLayout, pluginConfListener, webResourcesListener);
    ...
    if (rootFolder != null) {
      ...
      if (plugins != null) {
        for (File pluginRoot : plugins) {
          ...
          extractInfo(context, new NoJavaPluginLayout(pluginRoot), pluginConfListener,
  ...
  private void processJar(Context context, Object jar, ContextEntryListener listener) {
    InputStream jarStream = context.getLibAsStream(jar.toString());
    final ZipInputStream zis = new ZipInputStream(new BufferedInputStream(jarStream));
    ZipEntry entry;
    try {
  ...
  private void extractInfo(Context context, PluginLayout pluginLayout,
      PluginConfigEntryListener pluginConfListener,
      WebResourcesEntryListener webResourcesListener) {
    scanDir(context, pluginLayout.getWebResourcesRoot(), pluginLayout.getReferenceFolder(),
        pluginConfListener);
    scanDir(context, pluginLayout.getWebResourcesRoot(), pluginLayout.getReferenceFolder(),
        webResourcesListener);
  }

  private void scanDir(Context context, final File dir, final File referenceFolder,
      ContextEntryListener listener) {
    if (dir.isDirectory()) {
      Iterator<File> allFiles =
          FileUtils.iterateFiles(dir, relevantExtensions, TrueFileFilter.INSTANCE);
          ...
          ContextEntryReader contentReader = new ContextEntryReader() {
            ...
            public String getContent() throws IOException {
              InputStream input = new BufferedInputStream(
                  new FileInputStream(new File(referenceFolder, relativePath)));
  ...
  private interface ContextEntryListener {
    void accept(String path, ContextEntryReader contentReader) throws IOException;
  }
  ...
  private class PluginConfigEntryListener implements ContextEntryListener {
    ...
    @Override
    public void accept(String path, ContextEntryReader contentReader) throws IOException {
  ...
  private class WebResourcesEntryListener implements ContextEntryListener {
    ...
    public void accept(String path, ContextEntryReader contentReader) throws IOException {
      ...
      } else if ((path.startsWith(stylesPrefix) || path.startsWith(themePrefix))
          && path.endsWith(".css")) {
        String stylesheet = path.substring(pathPrefix.length());
        currentPlugin.addStylesheet(stylesheet);
      }
